...
public class PS2Connect implements StatConnect{
    ...
    private static final String CONTEXT = "/s:XouPs2/get/ps2/";
    ...
    public JSONObject findPlayerByName(String namePrefix) {
        try {
            JSONObject players = connection.establishConnectionAndQuery(CONTEXT+"character_name/","?name.first_lower=^" + namePrefix.toLowerCase() + "&c:limit=10");
            return connection.establishConnectionAndQuery(CONTEXT+"character","/?character_id=" + players.getJSONArray("character_name_list").getJSONObject(0).getString("character_id")).getJSONArray("character_list").getJSONObject(0);
        } catch (JSONException e) {
            Log.e(this.getClass().getName()+" FindPlayerByName", "Missong jsonArray from response.");
            throw new Error("Bad json");
        }
    }

    public JSONObject getCharacterStatsById() throws JSONException {
        return connection.establishConnectionAndQuery(CONTEXT+"character/","?character_id="+userID+"&c:resolve=stat_by_faction").getJSONArray("character_list").getJSONObject(0);
    }
    ...
    public List<String> listPlayersStartsWith(String prefix) {
        JSONObject players = connection.establishConnectionAndQuery(CONTEXT+"character_name/","?name.first_lower=^" + prefix.toLowerCase() + "&c:limit=10");
